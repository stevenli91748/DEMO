...
## master机上安装kubeadm等工具
         ...
         1.配置国内的kubernetes源：
          ...
          [root@master]# vi /etc/yum.repos.d/kubernetes.repo
                         ...
                         baseurl=http://yum.kubernetes.io/repos/kubernetes-el7-x86_64                  // 配置国外的kubernetes
                         enabled=1
                         gpgcheck=1
                         repo_gpgcheck=1
                         gpgkey=https://mirrors.aliyun.com/kubernetes/yum/doc/yum-key.gpg https://mirrors.aliyun.com
                         /kubernetes/yum/doc/rpm-package-key.gpg        //国内
         ...
         5. Kubernetes几乎所有的安装组件和Docker镜像都放在goolge自己的网站上,直接访问可能会有网络问题，这里的解决办法是从阿里云镜像仓库下载镜像，拉取到本地以后改回默认的镜像tag
           ...
           5.1 建立镜像下载的脚本
              // 因为无法直接访问gcr.io下载镜像，所以需要配置一个国内的容器镜像加速器,配置一个阿里云的加速器：登录 https://cr.console.aliyun.com/ 在页面中找到并点击镜像加速按钮，即可
                 看到属于自己的专属加速链接，选择Centos版本后即可看到配置方法
              ...
              [root@master]# more image.sh 
                             ...
                             version=v1.19.0                                           // 提示：镜像版本一定要和kubeadm安装的版本一致，否则会出现time out问题
                             images=(`kubeadm config images list --kubernetes-version=$version|awk -F '/' '{print $2}'`)
                             for imagename in ${images[@]} ; do
                               docker pull $url/$imagename
                               docker tag $url/$imagename k8s.gcr.io/$imagename     //由于kubeadrn 将自动下载Kubemetes 的相关镜像， 并且默认都是从gcr.io 进行下载的，
                                                                                      下载后将镜像名改为以gcr.io/ google_container 开头的名字， 以供kubeadrn 使用。
                               docker rmi -f $url/$imagename
                             done

              url为阿里云镜像仓库地址，version为安装的kubernetes版本
              
              上面的shell脚本主要做了3件事，下载各种需要用到的容器镜像、重新打标记为符合gcr.io/ google_container命令规范的版本名称、清除旧的容器镜像
...
**安装好kubeadm等相关工具后，接着在master虚拟机上执行以下操作：使用下面这条命令启动 master节点**
    ...
    3.1 初始化Master
        [root@master]# kubeadm init --kubernetes-version=v1.19.0    //提示：镜像版本一定要和镜像下载脚本(image.sh)的版本一致，否则会出现time out问题
  ...
  <a href="https://ibb.co/wMsPnZ7"><img src="https://i.ibb.co/6gBTKc8/kubeadm-token-number.jpg" alt="kubeadm-token-number" border="0"></a>
          ...
          初始化成功，要开始使用K8S集群的话，需要执行以下命令加载环境变量:
  ...
  到这里通过Kubeadm安装Kubernetes 集群已经成功了
