...
* [9. vagrant 打包](#9-打包)
* [10. LINUX下安装JDK](#10-LINUX下安装JDK)
* [11. 导入外部的文件到容器中和从容器中导出文件到外部](#11-导入外部的文件到容器中)

# 0 vagrant环境设置
* [VirtualBox网络配置超全详解](https://zhuanlan.zhihu.com/p/554893004?utm_id=0&wd=&eqid=f56701e60003b39a000000036497af01)
* [如何在Windows 11中启用Hyper-V](https://baijiahao.baidu.com/s?id=1777179585734801130&wfr=spider&for=pc)
...
CentOS 8| 192.168.33.185|Redis01|2|                  2G  |Redis01|
...
# 6 集群 SSH免密登录
* [在三台Centos虚拟机分主从机上设置SSH免密码登录](https://blog.csdn.net/2401_84297193/article/details/138433870?ops_request_misc=&request_id=&biz_id=102&utm_term=SSH%E5%85%8D%E5%AF%86%E7%99%BB%E5%BD%95%E9%85%8D%E7%BD%AE(%E4%B8%89%E5%8F%B0%E8%99%9A%E6%8B%9F%E6%9C%BA&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduweb~default-0-138433870.142^v100^pc_search_result_base3&spm=1018.2226.3001.4187)
* [Linux实现搭建ssh并允许使用root远程](https://www.jb51.net/server/315460xom.htm)
...
# 10 LINUX下安装JDK
     ...
     1） 先查看centos中自带的jdk并卸载
         ...
         [root]# rpm -qa | grep jdk   //查看
         [root]# rm -e | grep java    //删除
      ...
      2）yum 命令查找jdk所有版本 
         ...
         在这里，我选择了JAVA-1.8.0-openjdk-devel.x86_64 (OpenJDK 8 Development Environment)
         
       3) 安装jdk  
         
          [root] yum install -y JAVA-1.8.0-openjdk-devel.x86_64
       ...
       4) 设置jdk环境变量
          ...
          在文件最后加入如下配置：
                ...
                JAVA_HOME= /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.322.b06-2.el8_5.x86_64/jre
...
# 11 导入外部的文件到容器中         
      ...
      1） 在虚拟机mysql01上安装好docker
      
      2)  在docker中创建容器，以创建mysql-master01容器为例子

      3） 安装上传下载插件，并将Windows上的目录dockument/sql/mall.sql 上传到虚拟机 mysql01 的 目录 /SQL下 , 如果没有/SQL目录 ， 可自建
       
          [root]# yum -y install lrzsz
          
      4) 将mall.sql 文件拷贝到 mysql-master01容器的 /container/sql目录下
       
         [root]# docker exec -it mysql-master01 /bin/bash
         
         [root@mysql-master01]# mkdir /container/sql 
         
         [root@mysql-master01]# exit
      ...
      6） 将sql文件从mysql-master01容器导出到外部宿主机的 /SQL目录下       
      
         [root] docker cp mysql-master01:/container/sql/mall.sql /SQL
