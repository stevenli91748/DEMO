...
**[6. 配置root用户SSH免密码登录](https://weread.qq.com/web/reader/bd032100721bc56bbd056ffk4e73277021a4e732ced3b55)** 

如果需要搭建MySQL集群环境，为了避免登录每台服务器的烦琐操作，这里配置服务器的SSH免密码登录。后续只登录其中一台服务器，再通过SSH免密码登录到其他服务器即可

  [root] ssh-keygen -t rsa
  [root] cp ~/.ssh/id_rsa_.pub  ~/.ssh/authorized_keys

接下来，可以通过“ssh 主机名”或ssh ip的方式来验证SSH免密码登录是否配置成功

  [root] ssh gongzhou  or ssh 192.168.33.12
  
接下来退出通过SSH免密码登录的服务器会话终端
